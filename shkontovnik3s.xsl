...
<xsl:stylesheet version="1.0"
    ...
    <!-- shkontovnik3s.xsl
        ...
        K O N T O V N I K  pro sestavy se třemi vnořenými součty 
	 ...
	 - mezisoučty jsou ve třech urovnich vnořené v sobě 
	 - sloupce na konci tabulky jsou přesunuty k prvnímu (klíčovému )sloupci
	
	 2013-06-21 oh: uprava pro více sloupců
...
<!--  -->
   <xsl:key match="//SUBTOTAL[@level=2]/RS" name="ksloup" use="H[2]"/>
   ...
   <xsl:variable name="sloupce"  select="//SUBTOTAL[@level=2]" />
   <!-- seznam unikatnich nadpisu z druheho sloupce nesetrideny -->
   <xsl:variable name="unique" select="//SUBTOTAL[@level=2]/RS[count(.|key('ksloup',H[2])[1])=1]"/>
...
<!-- vlastni tabulka rolovaci -->
 ...
 <TABLE id="tbl">     
  <!-- zahlavi tabulky podle seznamu unique -->
    ...
    <TH  class="header"> <text>Celkem</text> </TH>	
    </TR>
    </THEAD>
   <TFOOT>
     <TR class="zapati">
     ...
     <TD class="zapati">
             <xsl:attribute name="colspan">
               <xsl:value-of select="count($unique)+count(//TH/HH)-2"/> 
             </xsl:attribute>
 ...
 <!-- pres soucty na urovni 1  -->
 <xsl:template match="NADPIS[SUBTOTAL/@level=1]">
        <xsl:if test="NAZEV">
          <TR >
           <TD colspan="3">
 ...
 <!-- pres mezisoucty  -->
 <xsl:template match="ZSUBTOTAL">
  <xsl:variable name="rs" select="." />
   <TR class="bg0">
     <TD class="row" > &#8721; : </TD>
     <!-- sloupce od 4 přidáme dopředu -->
       <TD class="row" >
         <xsl:attribute name="colspan"><xsl:value-of select="count(//TH/HH[position() &gt; 3])"/></xsl:attribute>
       &#160; </TD>
        <xsl:for-each select="$unique" >
        <xsl:sort select="H[2]"/>
        <TD class="rown">
	   <xsl:variable name="klicsloupce" select="H[2]"/> 
	   <xsl:variable name="value" select="$rs/RS[H[1]=$klicsloupce]/HN[1]"/> 
           <xsl:choose>
	    <xsl:when test="$local = '0'">
            	<xsl:value-of  select="format-number($value,'0.00')"/>
	     </xsl:when>
	     <xsl:otherwise>
            	<xsl:value-of  select="translate(format-number($value,'0.00'),'.',',')"/>
	     </xsl:otherwise>
	    </xsl:choose>
         <!--    RS=<xsl:value-of  select="$rs/RS[1]/H[1]"/>
           KLIC=<xsl:value-of  select="$klicsloupce"/>
            -->
        </TD>
	</xsl:for-each>
        ...
        <TD class="rown">
           <xsl:choose>
	    <xsl:when test="$local = '0'">
            	<xsl:value-of  select="format-number(sum(RS/HN[1]),'0.00')"/>
	     </xsl:when>
	     <xsl:otherwise>
            	<xsl:value-of  select="translate(format-number(sum(RS/HN[1]),'0.00'),'.',',')"/>
	     </xsl:otherwise>
	    </xsl:choose>
